...
def process_log_file(cur, filepath):
    ...
    for f in get_files(filepath):
        if 'checkpoint' not in f:
            df_from_each_file = pd.read_json(f, lines=True)
    ...
    time_data = pd.concat([t, t.dt.hour, t.dt.day, t.dt.weekofyear, t.dt.month, t.dt.year, t.dt.dayofweek], axis=1)
    column_labels = ["date", "hour", "day", "week", "month", "year", "weekday"]
    ...
    for index, row in df.iterrows():
        ...
        songplayid = str(row['ts']) + '-' + str(row['userId'])
        if results:
